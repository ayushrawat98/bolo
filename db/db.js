...
class Database {
	...
	constructor() {
		...
		this.queries = {
			...
			updateParentPostTime : this.database.prepare('update posts set updated_at = ? where id = ?'),
	...
	createChildPost(data) {
		const createThread = this.database.transaction((data) => {
			...
			this.queries.updateParentPostTime.run(new Date().toISOString(), parent.id)
